...
import com.bnd.core.runnable.{StateEvent, StateUpdatedEvent}
...
private final class InputLastTwoOutputsNetworkEvaluator[T](runTime : BigDecimal, evalFun : (Seq[T], Seq[T], Seq[T]) => Double) extends NetworkEvaluator[T] {
  ...
  private var lastState : Seq[T] = null
    ...
    event match {
        ...
        time : BigDecimal,
        components_ : Iterable[TopologicalNode],
        state : ju.List[T]
      ) => {
	      if (firstState == null) firstState = new ju.ArrayList(state).asScala.toSeq // clone()

	    	if (time.equals(runTime - 1))
	    	  lastButOneState = new ju.ArrayList(state).asScala.toSeq // clone()
	    	else if (time.equals(runTime))
	    	  lastState = new ju.ArrayList(state).asScala.toSeq // clone()
...
object NetworkEvaluator {
    ...
    private def customFunctionEvaluation[T : Manifest : DoubleConvertible](
      ...
      // TODO: this conversion is very ineffective
        val firstStateD : Seq[jl.Double] = firstState.map(x => new jl.Double(conv.toDouble(x)))
        val lastStateD : Seq[jl.Double] = if (lastState != null) lastState.map(x => new jl.Double(conv.toDouble(x))) else firstStateD
        val lastButOneStateD : Seq[jl.Double] = if (lastButOneState != null) lastButOneState.map(x => new jl.Double(conv.toDouble(x))) else lastStateD

        val environment = Map(
          "firstState" -> (firstStateD.toArray : Array[jl.Double]),
          "lastButOneState" -> (lastButOneStateD.toArray : Array[jl.Double]),
          "lastState" -> (lastStateD.toArray : Array[jl.Double])
        )

        val componentIndexMap = Map("firstState" -> 0, "lastButOneState" -> 1, "lastState" -> 2)
        val variableFunIndexMap : Map[NetworkEvaluationVariable, Int] = networkEvaluation.getVariables.asScala.map(
        ...
        val varEnvironment = networkEvaluation.getEvaluationItems.asScala.map(item => {
    ...
    def apply[T : Manifest : DoubleConvertible](
      ...
      new InputLastTwoOutputsNetworkEvaluator(
        runTime,
        customFunctionEvaluation[T](funEvaluatorFactory, networkEvaluation)
      )
