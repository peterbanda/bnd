...
private class NetworkDerridaAnalyserImpl[T](
    ...
    override def run(
        ...
        val orderedComponents1 = new ju.ArrayList[TopologicalNode](components1.asJavaCollection)
		if (components1.head.hasLocation)
			Collections.sort(orderedComponents1, new TopologicalNodeLocationComparator)

		val orderedComponents2 = new ju.ArrayList[TopologicalNode](components2.asJavaCollection)
        ...
        def runNetwork(
		    ...
		    (orderedComponents.asScala.zip(initState.asScala)).foreach{
		        case (component,state) => networkRunnable.setState(component, state)
		    }
	    	networkRunnable.runFor(runTime)
	    	orderedComponents.asScala.map(networkRunnable.getState(_)).toList
        }
        ...
        val distances = for (perturbPos <- 0 to networkSize) yield
	    	for (i <- 1 to repetitions) yield {
	    		...
	    		positions.asScala.foreach(pos => initState2.set(pos, flip(initState2.get(pos))))

	    		val initDistance = distanceFun(initState1.asScala.toSeq, initState2.asScala.toSeq)
