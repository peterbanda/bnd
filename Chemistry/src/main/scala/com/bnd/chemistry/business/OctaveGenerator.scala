...
object OctaveGenerator {
	...
	def apply(reactionSet : AcReactionSet) = if (reactionSet == null) "" else {
        ...
        val speciesLabelAndExpressions = speciesExpressionMap.toList.map {
            case(species, builder) => 
            	(species.getLabel, species.getLabel + "dot = " + builder)
				}.sortBy(_._1)

       	val listedSpecies = speciesLabelAndExpressions.map(_._1).mkString(", ")
	...
	private def createSpeciesExpressionMap(reactionSet : AcReactionSet) = { 	   
	    val speciesExpressionMap = reactionSet.getSpecies().asScala.map((_, new StringBuilder)).toMap
	    for (reaction <- reactionSet.getReactions.asScala) yield {
	    	...
	    	// reactants
	    	reaction.getSpeciesAssociations(AcSpeciesAssociationType.Reactant).asScala.foreach(assoc =>
	    	...
	    	// products
	    	reaction.getSpeciesAssociations(AcSpeciesAssociationType.Product).asScala.foreach(assoc =>
	    	   	speciesExpressionMap(assoc.getSpecies).append(
	    	   			if (assoc.getStoichiometricFactor.equals(1d))
	    	   				" + " + expression
	    	   			else
