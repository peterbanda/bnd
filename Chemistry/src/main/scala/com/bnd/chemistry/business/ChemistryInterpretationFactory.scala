...
import scala.collection.mutable.{ListBuffer, Publisher => ScalaPublisher}
import com.bnd.core.runnable.{TimeRunnable, StateEvent, StateInterpreter, StateInterpretationEvent, StateInterpretationResultCollector, SeqIndexAccessible}
...
class ChemistryInterpretationFactory(private val functionEvaluatorFactory : FunctionEvaluatorFactory) extends Serializable {
    ...
    type StatePublisher[S[X]] = ScalaPublisher[StateEvent[jl.Double, S]]
    ...
    def apply[S[X] : SeqIndexAccessible](
        ...
        val interpreter = StateInterpreter(chemistry, statePublisher, interpretations, Some((translationSeries.getVariables().asScala : Iterable[AcTranslationVariable], translationSeries.getDefaultVariableValue)))
        return new ChemistryInterpretation(interpreter, translationSeries)
	}
}

private class ChemistryInterpretation(
    chemistryInterpreter : TimeRunnable with ScalaPublisher[StateInterpretationEvent[jl.Double, AcSpecies, AcTranslationVariable]],
    translationSeries : AcTranslationSeries) extends TimeRunnable with AcTranslatedRunHolder {

    val variableMap = translationSeries.getVariables.asScala.map(v => (v.getLabel, v)).toMap
    val interpretedValuesMap = translationSeries.getVariables.asScala.map(v => (v, ListBuffer[jl.Double]())).toMap
    ...
    override def getAcTranslatedRun = {
        ...
        for ((variable, interpretedValues) <- interpretedValuesMap) { 
        	...
        	history.setSequence(new ju.ArrayList[jl.Double](interpretedValues.asJava))
